...
func (provider PostgresSharedProvider) GetInstance(name string, plan *ProviderPlan) (*DbInstance, error) {
	...
	return &DbInstance{
		...
		Username:"",
		Password:"",
		Endpoint:settings.Host + ":" + settings.Port + "/" + name,
		Status:"available",
		Ready:true,
		Engine:"postgres", 
...
func (provider PostgresSharedProvider) Provision(Id string, plan *ProviderPlan, Owner string) (*DbInstance, error) {
	...
	db, err := sql.Open("postgres", "postgres://" + settings.MasterUsername + ":" + settings.MasterPassword + "@" + settings.Host + ":" + settings.Port)
	if err != nil {
	...
	if _, err = db.Exec("GRANT " + username + " TO " + settings.MasterUsername); err != nil {
	...
	udb, err := sql.Open("postgres", "postgres://" + settings.MasterUsername + ":" + settings.MasterUsername + "@" + settings.Host + ":" + settings.Port + "/" + db_name)
	if err != nil {
		return nil, err
	}
	defer udb.Close()
	...
	return &DbInstance{
		...
		Username:username,
		Password:password,
		Endpoint:settings.Host + ":" + settings.Port + "/" + db_name,
		Status:"available",
		Ready:true,
...
func (provider PostgresSharedProvider) Deprovision(dbInstance *DbInstance, takeSnapshot bool) (error) {
	...
	if _, err = db.Exec("REVOKE " + dbInstance.Username + " FROM " + settings.MasterUsername); err != nil {
