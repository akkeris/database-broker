...
begin
    ...
    if not exists (select 1 from pg_type where typname = 'clientdbtype') then
        create type clientdbtype as enum('postgres', 'mysqlx', 'oracledb', 'mssql');
    end if;
    ...
    if (select count(*) from services) = 0 then
        ...
        values 
            ('01bb60d2-f2bb-64c0-4c8b-ead731a690bc','alamo-postgresql', 'Akkeris PostgreSQL', 'Dedicated and scalable PostgreSQL relational SQL database.', 'Data Stores,postgres', 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/Postgresql_elephant.svg/1200px-Postgresql_elephant.svg.png', false, false);
    end if;
    ...
    if (select count(*) from plans) = 0 then
        -- 9 plans
        insert into plans 
            (plan, service, name, human_name, description, version, type, scheme, categories, cost_cents, attributes, provider, provider_private_details)
        values 
            ('50660450-61d3-2c13-a3fd-d379997932fb', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'hobby-v9',      'Hobby (9.6)',        'Postgres 9.6.6 - 1xCPU 1GB Ram 512MB Storage', '9.6.6', 'postgres', 'postgres', 'Data Stores', 0,     '{"compliance":"", "supports_extensions":false, "ram":"1GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"512MB", "data_clips":false, "connection_limit":20,   "high_availability":false,  "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":false, "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_9_USERNAME}", "master_password":"${PG_HOBBY_9_PASSWORD}", "host":"${PG_HOBBY_9_HOST}", "port":"${PG_HOBBY_9_PORT}", "engine":"postgres", "engine_version":"9.6.6"}'),
            ('a329070d-8caa-2c1e-1897-2c6b15907845', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'standard-0-v9', 'Standard-0 (9.6)',   'Postgres 9.6.6 - 1xCPU 2GB Ram 4GB Storage',   '9.6.6', 'postgres', 'postgres', 'Data Stores', 500,   '{"compliance":"", "supports_extensions":false, "ram":"2GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"4GB",   "data_clips":false, "connection_limit":50,   "high_availability":true,   "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":false, "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_9_USERNAME}", "master_password":"${PG_HOBBY_9_PASSWORD}", "host":"${PG_HOBBY_9_HOST}", "port":"${PG_HOBBY_9_PORT}", "engine":"postgres", "engine_version":"9.6.6"}'),
            ('c4330196-7d00-b529-4217-edc9c4b7b2d4', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'standard-1-v9', 'Standard-1 (9.6)',   'Postgres 9.6.6 - 1xCPU 2GB Ram 16GB Storage',  '9.6.6', 'postgres', 'postgres', 'Data Stores', 1500,  '{"compliance":"", "supports_extensions":false, "ram":"2GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"16GB",  "data_clips":false, "connection_limit":100,  "high_availability":true,   "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":false, "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_9_USERNAME}", "master_password":"${PG_HOBBY_9_PASSWORD}", "host":"${PG_HOBBY_9_HOST}", "port":"${PG_HOBBY_9_PORT}", "engine":"postgres", "engine_version":"9.6.6"}'),
            ('553eebe4-62f4-4b2d-e24e-39da5f31a713', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'standard-2-v9', 'Standard-2 (9.6)',   'Postgres 9.6.6 - 1xCPU 4GB Ram 32GB Storage',  '9.6.6', 'postgres', 'postgres', 'Data Stores', 4500,  '{"compliance":"", "supports_extensions":false, "ram":"4GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"32GB",  "data_clips":false, "connection_limit":200,  "high_availability":true,   "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_9_USERNAME}", "master_password":"${PG_HOBBY_9_PASSWORD}", "host":"${PG_HOBBY_9_HOST}", "port":"${PG_HOBBY_9_PORT}", "engine":"postgres", "engine_version":"9.6.6"}'),
            ('d294d7f1-19b4-f72c-e3bd-96970f74f027', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'premium-0-v9',  'Premium-1 (9.5)',    'Postgres 9.5.2 - 2xCPU 4GB Ram 20GB Storage',  '9.5.2', 'postgres', 'postgres', 'Data Stores', 6000,  '{"compliance":"", "supports_extensions":true,  "ram":"4GB",   "database_replicas":true,  "database_logs":true,  "restartable":true,  "row_limits":null, "storage_capacity":"20GB",  "data_clips":true,  "connection_limit":null, "high_availability":false,  "rollback":"14 days", "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":false, "dedicated":true  }', 'aws-instance',    '{"AllocatedStorage":20,"AutoMinorVersionUpgrade":true,"AvailabilityZone":null,"BackupRetentionPeriod":14,"CharacterSetName":null,"CopyTagsToSnapshot":true,"DBClusterIdentifier":null,"DBInstanceClass":"db.t2.medium","DBInstanceIdentifier":null,"DBName":null,"DBParameterGroupName":null,"DBSecurityGroups":null,"DBSubnetGroupName":null,"Domain":null,"DomainIAMRoleName":null,"EnableCloudwatchLogsExports":null,"EnableIAMDatabaseAuthentication":null,"EnablePerformanceInsights":false,"Engine":"postgres","EngineVersion":"9.6.9","Iops":null,"KmsKeyId":null,"LicenseModel":null,"MasterUserPassword":null,"MasterUsername":null,"MonitoringInterval":null,"MonitoringRoleArn":null,"MultiAZ":false,"OptionGroupName":null,"PerformanceInsightsKMSKeyId":null,"PerformanceInsightsRetentionPeriod":null,"Port":null,"PreferredBackupWindow":null,"PreferredMaintenanceWindow":null,"ProcessorFeatures":null,"PromotionTier":null,"PubliclyAccessible":false,"StorageEncrypted":true,"StorageType":"gp2","Tags":null,"TdeCredentialArn":null,"TdeCredentialPassword":null,"Timezone":null,"VpcSecurityGroupIds":null}'),
            ('682631dd-0374-e495-f1f6-f16406962873', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'premium-1-v9',  'Premium-2 (9.5)',    'Postgres 9.5.2 - 2xCPU 8GB Ram 50GB Storage',  '9.5.2', 'postgres', 'postgres', 'Data Stores', 13500, '{"compliance":"", "supports_extensions":true,  "ram":"8GB",   "database_replicas":true,  "database_logs":true,  "restartable":true,  "row_limits":null, "storage_capacity":"50GB",  "data_clips":true,  "connection_limit":null, "high_availability":false,  "rollback":"14 days", "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":false, "dedicated":true  }', 'aws-instance',    '{"AllocatedStorage":50,"AutoMinorVersionUpgrade":true,"AvailabilityZone":null,"BackupRetentionPeriod":14,"CharacterSetName":null,"CopyTagsToSnapshot":true,"DBClusterIdentifier":null,"DBInstanceClass":"db.m4.large","DBInstanceIdentifier":null,"DBName":null,"DBParameterGroupName":null,"DBSecurityGroups":null,"DBSubnetGroupName":null,"Domain":null,"DomainIAMRoleName":null,"EnableCloudwatchLogsExports":null,"EnableIAMDatabaseAuthentication":null,"EnablePerformanceInsights":true,"Engine":"postgres","EngineVersion":"9.6.9","Iops":null,"KmsKeyId":null,"LicenseModel":null,"MasterUserPassword":null,"MasterUsername":null,"MonitoringInterval":null,"MonitoringRoleArn":null,"MultiAZ":false,"OptionGroupName":null,"PerformanceInsightsKMSKeyId":null,"PerformanceInsightsRetentionPeriod":7,"Port":null,"PreferredBackupWindow":null,"PreferredMaintenanceWindow":null,"ProcessorFeatures":null,"PromotionTier":null,"PubliclyAccessible":false,"StorageEncrypted":true,"StorageType":"gp2","Tags":null,"TdeCredentialArn":null,"TdeCredentialPassword":null,"Timezone":null,"VpcSecurityGroupIds":null}'),
            ('53c42a36-e61d-6fa4-7d89-1c774aeec016', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'premium-2-v9',  'Premium-3 (9.5)',    'Postgres 9.5.2 - 4xCPU 16GB Ram 100GB Storage','9.5.2', 'postgres', 'postgres', 'Data Stores', 72000, '{"compliance":"", "supports_extensions":true,  "ram":"16GB",  "database_replicas":true,  "database_logs":true,  "restartable":true,  "row_limits":null, "storage_capacity":"100GB", "data_clips":true,  "connection_limit":null, "high_availability":true,   "rollback":"14 days", "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":false, "dedicated":true  }', 'aws-instance',    '{"AllocatedStorage":100,"AutoMinorVersionUpgrade":true,"AvailabilityZone":null,"BackupRetentionPeriod":14,"CharacterSetName":null,"CopyTagsToSnapshot":true,"DBClusterIdentifier":null,"DBInstanceClass":"db.m4.large","DBInstanceIdentifier":null,"DBName":null,"DBParameterGroupName":null,"DBSecurityGroups":null,"DBSubnetGroupName":null,"Domain":null,"DomainIAMRoleName":null,"EnableCloudwatchLogsExports":null,"EnableIAMDatabaseAuthentication":null,"EnablePerformanceInsights":true,"Engine":"postgres","EngineVersion":"9.6.9","Iops":1000,"KmsKeyId":null,"LicenseModel":null,"MasterUserPassword":null,"MasterUsername":null,"MonitoringInterval":null,"MonitoringRoleArn":null,"MultiAZ":true,"OptionGroupName":null,"PerformanceInsightsKMSKeyId":null,"PerformanceInsightsRetentionPeriod":7,"Port":null,"PreferredBackupWindow":null,"PreferredMaintenanceWindow":null,"ProcessorFeatures":null,"PromotionTier":null,"PubliclyAccessible":false,"StorageEncrypted":true,"StorageType":"io1","Tags":null,"TdeCredentialArn":null,"TdeCredentialPassword":null,"Timezone":null,"VpcSecurityGroupIds":null}');

        -- 10 plans
        insert into plans 
            (plan, service, name, human_name, description, version, type, scheme, categories, cost_cents, attributes, provider, provider_private_details)
        values 
            ('a0660450-61d3-2c13-a3fd-d379997932f1', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'hobby',         'Hobby (10.4)',        'Postgres 10.4 - 1xCPU 1GB Ram 512MB Storage', '10.4', 'postgres', 'postgres', 'Data Stores', 0,     '{"compliance":"", "supports_extensions":false, "ram":"1GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"512MB", "data_clips":false, "connection_limit":20,   "high_availability":false,  "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":false, "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_10_USERNAME}", "master_password":"${PG_HOBBY_10_PASSWORD}", "host":"${PG_HOBBY_10_HOST}", "port":"${PG_HOBBY_10_PORT}", "engine":"postgres", "engine_version":"10.4"}'),
            ('b329070d-8caa-2c1e-1897-2c6b15907842', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'standard-0',    'Standard-0 (10.4)',   'Postgres 10.4 - 1xCPU 2GB Ram 4GB Storage',   '10.4', 'postgres', 'postgres', 'Data Stores', 500,   '{"compliance":"", "supports_extensions":false, "ram":"2GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"4GB",   "data_clips":false, "connection_limit":50,   "high_availability":true,   "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":false, "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_10_USERNAME}", "master_password":"${PG_HOBBY_10_PASSWORD}", "host":"${PG_HOBBY_10_HOST}", "port":"${PG_HOBBY_10_PORT}", "engine":"postgres", "engine_version":"10.4"}'),
            ('c4330196-7d00-b529-4217-edc9c4b7b2d3', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'standard-1',    'Standard-1 (10.4)',   'Postgres 10.4 - 1xCPU 2GB Ram 16GB Storage',  '10.4', 'postgres', 'postgres', 'Data Stores', 1500,  '{"compliance":"", "supports_extensions":false, "ram":"2GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"16GB",  "data_clips":false, "connection_limit":100,  "high_availability":true,   "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":false, "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_10_USERNAME}", "master_password":"${PG_HOBBY_10_PASSWORD}", "host":"${PG_HOBBY_10_HOST}", "port":"${PG_HOBBY_10_PORT}", "engine":"postgres", "engine_version":"10.4"}'),
            ('d53eebe4-62f4-4b2d-e24e-39da5f31a714', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'standard-2',    'Standard-2 (10.4)',   'Postgres 10.4 - 1xCPU 4GB Ram 32GB Storage',  '10.4', 'postgres', 'postgres', 'Data Stores', 4500,  '{"compliance":"", "supports_extensions":false, "ram":"4GB",   "database_replicas":false, "database_logs":false, "restartable":false, "row_limits":null, "storage_capacity":"32GB",  "data_clips":false, "connection_limit":200,  "high_availability":true,   "rollback":"7 days",  "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":true,  "dedicated":false }', 'postgres-shared', '{"master_username":"${PG_HOBBY_10_USERNAME}", "master_password":"${PG_HOBBY_10_PASSWORD}", "host":"${PG_HOBBY_10_HOST}", "port":"${PG_HOBBY_10_PORT}", "engine":"postgres", "engine_version":"10.4"}'),
            ('e294d7f1-19b4-f72c-e3bd-96970f74f025', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'premium-0',     'Premium-1 (10.4)',    'Postgres 10.4 - 2xCPU 4GB Ram 20GB Storage',  '10.4', 'postgres', 'postgres', 'Data Stores', 6000,  '{"compliance":"", "supports_extensions":true,  "ram":"4GB",   "database_replicas":true,  "database_logs":true,  "restartable":true,  "row_limits":null, "storage_capacity":"20GB",  "data_clips":true,  "connection_limit":null, "high_availability":false,  "rollback":"14 days", "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":false, "dedicated":true  }', 'aws-instance',    '{"AllocatedStorage":20,"AutoMinorVersionUpgrade":true,"AvailabilityZone":null,"BackupRetentionPeriod":14,"CharacterSetName":null,"CopyTagsToSnapshot":true,"DBClusterIdentifier":null,"DBInstanceClass":"db.t2.medium","DBInstanceIdentifier":null,"DBName":null,"DBParameterGroupName":null,"DBSecurityGroups":null,"DBSubnetGroupName":null,"Domain":null,"DomainIAMRoleName":null,"EnableCloudwatchLogsExports":null,"EnableIAMDatabaseAuthentication":null,"EnablePerformanceInsights":false,"Engine":"postgres","EngineVersion":"10.4","Iops":null,"KmsKeyId":null,"LicenseModel":null,"MasterUserPassword":null,"MasterUsername":null,"MonitoringInterval":null,"MonitoringRoleArn":null,"MultiAZ":false,"OptionGroupName":null,"PerformanceInsightsKMSKeyId":null,"PerformanceInsightsRetentionPeriod":null,"Port":null,"PreferredBackupWindow":null,"PreferredMaintenanceWindow":null,"ProcessorFeatures":null,"PromotionTier":null,"PubliclyAccessible":false,"StorageEncrypted":true,"StorageType":"gp2","Tags":null,"TdeCredentialArn":null,"TdeCredentialPassword":null,"Timezone":null,"VpcSecurityGroupIds":null}' ),
            ('f82631dd-0374-e495-f1f6-f16406962876', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'premium-1',     'Premium-2 (10.4)',    'Postgres 10.4 - 2xCPU 8GB Ram 50GB Storage',  '10.4', 'postgres', 'postgres', 'Data Stores', 13500, '{"compliance":"", "supports_extensions":true,  "ram":"8GB",   "database_replicas":true,  "database_logs":true,  "restartable":true,  "row_limits":null, "storage_capacity":"50GB",  "data_clips":true,  "connection_limit":null, "high_availability":false,  "rollback":"14 days", "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":false, "dedicated":true  }', 'aws-instance',    '{"AllocatedStorage":50,"AutoMinorVersionUpgrade":true,"AvailabilityZone":null,"BackupRetentionPeriod":14,"CharacterSetName":null,"CopyTagsToSnapshot":true,"DBClusterIdentifier":null,"DBInstanceClass":"db.m4.large","DBInstanceIdentifier":null,"DBName":null,"DBParameterGroupName":null,"DBSecurityGroups":null,"DBSubnetGroupName":null,"Domain":null,"DomainIAMRoleName":null,"EnableCloudwatchLogsExports":null,"EnableIAMDatabaseAuthentication":null,"EnablePerformanceInsights":true,"Engine":"postgres","EngineVersion":"10.4","Iops":null,"KmsKeyId":null,"LicenseModel":null,"MasterUserPassword":null,"MasterUsername":null,"MonitoringInterval":null,"MonitoringRoleArn":null,"MultiAZ":false,"OptionGroupName":null,"PerformanceInsightsKMSKeyId":null,"PerformanceInsightsRetentionPeriod":7,"Port":null,"PreferredBackupWindow":null,"PreferredMaintenanceWindow":null,"ProcessorFeatures":null,"PromotionTier":null,"PubliclyAccessible":false,"StorageEncrypted":true,"StorageType":"gp2","Tags":null,"TdeCredentialArn":null,"TdeCredentialPassword":null,"Timezone":null,"VpcSecurityGroupIds":null}' ),
            ('13c42a36-e61d-6fa4-7d89-1c774aeec017', '01bb60d2-f2bb-64c0-4c8b-ead731a690bc', 'premium-2',     'Premium-3 (10.4)',    'Postgres 10.4 - 4xCPU 16GB Ram 100GB Storage','10.4', 'postgres', 'postgres', 'Data Stores', 72000, '{"compliance":"", "supports_extensions":true,  "ram":"16GB",  "database_replicas":true,  "database_logs":true,  "restartable":true,  "row_limits":null, "storage_capacity":"100GB", "data_clips":true,  "connection_limit":null, "high_availability":true,   "rollback":"14 days", "encryption_at_rest":true, "high_speed_ssd":true,  "burstable_performance":false, "dedicated":true  }', 'aws-instance',    '{"AllocatedStorage":100,"AutoMinorVersionUpgrade":true,"AvailabilityZone":null,"BackupRetentionPeriod":14,"CharacterSetName":null,"CopyTagsToSnapshot":true,"DBClusterIdentifier":null,"DBInstanceClass":"db.m4.large","DBInstanceIdentifier":null,"DBName":null,"DBParameterGroupName":null,"DBSecurityGroups":null,"DBSubnetGroupName":null,"Domain":null,"DomainIAMRoleName":null,"EnableCloudwatchLogsExports":null,"EnableIAMDatabaseAuthentication":null,"EnablePerformanceInsights":true,"Engine":"postgres","EngineVersion":"10.4","Iops":1000,"KmsKeyId":null,"LicenseModel":null,"MasterUserPassword":null,"MasterUsername":null,"MonitoringInterval":null,"MonitoringRoleArn":null,"MultiAZ":true,"OptionGroupName":null,"PerformanceInsightsKMSKeyId":null,"PerformanceInsightsRetentionPeriod":7,"Port":null,"PreferredBackupWindow":null,"PreferredMaintenanceWindow":null,"ProcessorFeatures":null,"PromotionTier":null,"PubliclyAccessible":false,"StorageEncrypted":true,"StorageType":"io1","Tags":null,"TdeCredentialArn":null,"TdeCredentialPassword":null,"Timezone":null,"VpcSecurityGroupIds":null}');
    end if;
end
$$
`;
...
func (b *PostgresStorage) GetPlanByID(planId string) (*ProviderPlan, error) {
    plans, err := b.getPlans(" and plans.plan = $1", planId)
    if err != nil {
        return nil, err
    }
    return &plans[0], nil
...
func (b *PostgresStorage) GetPlans(serviceId string) ([]ProviderPlan, error) {
    return b.getPlans(" and services.service = $1", serviceId)
}
